(function (root) {
  ...
  var RULE_CASE_ELSE_DATATYPE = {
    id: 'case-else-datatype', matchError: /inconsistent datatype|conversion failed|cannot convert|type mismatch|invalid.*use of|ORA-00932/i,
    apply: function (sql, ctx) {
      ...
      return { sql: newSql, identified: 'The CASE expression returns text values in the THEN clauses but returns a non-text column value directly in the ELSE clause, which most databases reject as an inconsistent-datatype error.', applied: 'The ELSE clause has been updated to convert the column to text (using the syntax appropriate for the ' + ctx.dialect + ' dialect) so that every branch of the CASE expression returns a compatible data type.', changes: changes };
    }
  };
  var RULE_WHERE_DATATYPE = {
    id: 'where-comparison-datatype', matchError: /inconsistent datatype|conversion failed|cannot convert|type mismatch|incorrect syntax near|operator does not exist|ORA-00932/i,
    apply: function (sql, ctx) {
      ...
      return { sql: newSql, identified: 'A WHERE condition compares a numeric column against a quoted text literal, which some databases reject as an inconsistent-datatype error.', applied: 'The quotes have been removed from the numeric literal so the comparison uses matching data types on both sides.', changes: changes };
    }
  };
  var RULE_INVALID_COLUMN = {
    id: 'invalid-column', matchError: /invalid identifier|invalid column name|column .* does not exist|unknown column|ORA-00904/i,
    apply: function (sql, ctx) {
      ...
      return { sql: newSql, identified: 'The column "' + bad + '" referenced in the SQL does not appear to exist in the active schema for the table(s) used in this query.', applied: 'Replaced "' + bad + '" with "' + suggestion + '", the closest matching column name found in the active schema for the table(s) in this query.', changes: [{ from: bad, to: suggestion }] };
    }
  };
  var RULE_INVALID_TABLE = {
    id: 'invalid-table', matchError: /table or view does not exist|invalid object name|relation .* does not exist|doesn.t exist|ORA-00942/i,
    apply: function (sql, ctx) {
      var idMatch = ctx.errorText.match(/"([A-Za-z_][\w]*)"|'([A-Za-z_][\w]*)"?|\[([A-Za-z_][\w]*)\]|`([A-Za-z_][\w]*)`/);
  ...
  function splitTopLevel(text) {
    ...
    for (var i = 0; i < text.length; i++) { var ch = text[i]; if (ch === '(') depth++; if (ch === ')') depth--; if (ch === ',' && depth === 0) { parts.push(current); current = ''; } else current += ch; }
    if (current.trim()) parts.push(current);
    return parts;
  }
  var RULE_GROUP_BY = {
    id: 'group-by-missing-column', matchError: /not a group by expression|not contained in either an aggregate function or the group by clause|must appear in the group by clause|ORA-00979/i,
    apply: function (sql, ctx) {
      ...
      var newSql;
      if (groupByMatch) { var oldClause = 'GROUP BY' + groupByMatch[1]; var newClause = 'GROUP BY' + groupByMatch[1].replace(/\s*$/, '') + ', ' + missing.join(', '); newSql = sql.replace(oldClause, newClause); }
      else { var insertAfter = sql.match(/\bFROM\b[\s\S]*?(?=\bWHERE\b|\bORDER BY\b|;|$)/i); if (!insertAfter) return null; newSql = sql.slice(0, insertAfter.index + insertAfter[0].length) + '\nGROUP BY ' + nonAggCols.join(', ') + sql.slice(insertAfter.index + insertAfter[0].length); }
      return { sql: newSql, identified: 'The SELECT list includes column(s) that are neither wrapped in an aggregate function (COUNT/SUM/AVG/MIN/MAX) nor listed in the GROUP BY clause.', applied: 'Added the missing column(s) \u2014 ' + missing.join(', ') + ' \u2014 to the GROUP BY clause so every non-aggregated SELECT column is accounted for.', changes: [{ from: groupByMatch ? ('GROUP BY' + groupByMatch[1]) : '(no GROUP BY clause)', to: 'GROUP BY ' + (groupByMatch ? existingGroupCols.concat(missing).join(', ') : nonAggCols.join(', ')) }] };
    }
  };
  var RULE_DATE_FORMAT = {
    id: 'date-format', matchError: /does not match the format string|literal does not match|conversion failed when converting date|invalid datetime format|date\/time field value out of range|ORA-01861|ORA-01858/i,
    apply: function (sql, ctx) {
      if (!DATATYPE) return null;
      var cmpRe = /([A-Za-z_][\w]*\.)?([A-Za-z_][\w]*)\s*(=|>=|<=|>|<)\s*'([^']*)'/g;
      var changes = []; var newSql = sql; var m;
      while ((m = cmpRe.exec(sql))) {
        ...
        if (newSql.indexOf(fromFragment) !== -1 && fromFragment !== toFragment) { newSql = newSql.replace(fromFragment, toFragment); changes.push({ from: fromFragment, to: toFragment }); }
      }
      if (!changes.length) return null;
      return { sql: newSql, identified: 'A date/timestamp column is being compared against a plain text literal, which some databases cannot implicitly interpret as a date.', applied: 'Wrapped the date literal(s) in the date-parsing syntax appropriate for the ' + ctx.dialect + ' dialect.', changes: changes };
    }
  };
  var RULE_NULL_COMPARISON = {
    ...
    apply: function (sql) {
      ...
      if (!changes.length) return null;
      return { sql: newSql, identified: 'The SQL compares a column to NULL using "=" or "<>", which does not work as expected in standard SQL (NULL is never equal or unequal to anything, including itself).', applied: 'Rewrote the comparison(s) to use "IS NULL" / "IS NOT NULL", which is the correct way to test for NULL in SQL.', changes: changes };
    }
  };
  var RULE_TRAILING_COMMA = {
    ...
    apply: function (sql) {
      ...
      if (!changes.length) return null;
      return { sql: newSql, identified: 'The SQL contains a stray comma immediately before a keyword or closing parenthesis, which most databases treat as a syntax error.', applied: 'Removed the stray trailing comma.', changes: changes };
    }
  };
  ...
  var RULE_NULLFN_DIALECT = {
    id: 'nullfn-dialect', matchError: /invalid identifier|is not a recognized|function .* does not exist|unknown function|not a valid function|invalid procedure or function|ORA-00904/i, alwaysTry: true,
    apply: function (sql, ctx) {
      var correct = NULLFN_BY_DIALECT[ctx.dialect] || 'COALESCE';
      var changes = []; var newSql = sql;
      ALL_NULLFNS.forEach(function (fn) {
        if (fn === correct) return;
        var re = new RegExp('\\b' + fn + '\\s*\\(', 'gi'); var m;
        while ((m = re.exec(sql))) {
          ...
          var fromFragment = fn + '(' + argText + ')'; var toFragment = correct + '(' + argText + ')';
          if (newSql.indexOf(fromFragment) !== -1) { newSql = newSql.replace(fromFragment, toFragment); changes.push({ from: fromFragment, to: toFragment }); }
        }
      });
      if (!changes.length) return null;
      return { sql: newSql, identified: 'The SQL uses a NULL-handling function that is not available in the selected SQL dialect (' + ctx.dialect + ').', applied: 'Replaced it with ' + correct + '(...), the equivalent function for the ' + ctx.dialect + ' dialect.', changes: changes };
    }
  };
  var RULE_JOIN_RELATIONSHIP = {
    ...
    apply: function (sql, ctx) {
      ...
      return { sql: newSql, identified: 'The JOIN condition references a column that does not exist on one of the joined tables (' + leftTable + '.' + leftCol + ' / ' + rightTable + '.' + rightCol + ').', applied: 'Replaced the join condition with the relationship documented in the active schema between ' + leftTable + ' and ' + rightTable + '.', changes: [{ from: fromFragment, to: toFragment }] };
    }
  };
  ...
  function rectify(sql, errorText, engine, dialect) {
    ...
    if (!sql.trim()) return { correctedSql: sql, changed: false, errorIdentified: 'No SQL was supplied to analyze.', correctionApplied: 'Please paste the SQL query that produced the error in Box 2, then try again.', changes: [], ruleId: null };
    ...
    return { correctedSql: sql, changed: false, errorIdentified: 'No specific, schema-supported correction could be automatically determined for this error from the supplied SQL and the active schema.', correctionApplied: 'No automatic change was made, to avoid guessing. Please review the SQL and error manually, double-check the table/column names against Used Schema, or consult your database documentation.', changes: [], ruleId: null };
  }
