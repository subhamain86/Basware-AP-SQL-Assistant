(function (root) {
  ...
  function resolveColumnDisplay(engine, decodeStore, col, dialect) {
    if (col.aggregate) {
      ...
      var expr = col.aggregate + '(' + (col.distinct ? 'DISTINCT ' : '') + target + ')';
    ...
    if (col.decode) {
      ...
      if (resolved.values && resolved.values.length) {
        var schemaCol = engine.getColumn(col.table, col.column);
        var dataType = schemaCol ? schemaCol.type : null;
        return DECODE.buildDecodeCaseSql(col.table, col.column, resolved.values, col.alias || col.column, { dataType: dataType, dialect: dialect, elseMode: col.elseMode });
      }
    }
  ...
  function generateSql(requirement, options, engine, decodeStore) {
    ...
    var filtersAppliedEarly = [];
    var usedScalarAliases = {};
    function addScalarSubquery(sub) {
      ...
      var aggFunc = sub.aggFunc || 'COUNT';
      var scalarAlias = sub.alias || (sub.relatedTable + '_' + aggFunc.toLowerCase());
    ...
    if (options.scalarSubquery && options.scalarSubquery.relatedTable) {
      var scalarErr = addScalarSubquery(options.scalarSubquery);
      if (scalarErr) return { status: 'rejected', message: scalarErr };
    }
    if (Array.isArray(options.scalarSubqueries)) {
      for (var si = 0; si < options.scalarSubqueries.length; si++) {
        var scalarErr2 = addScalarSubquery(options.scalarSubqueries[si]);
        if (scalarErr2) return { status: 'rejected', message: scalarErr2 };
      }
    }
    var lines = [];
    var distinctKw = options.distinct ? 'DISTINCT ' : '';
    ...
    var whereParts = [];
    if (options.filterGroup && options.filterGroup.conditions && options.filterGroup.conditions.length) {
      var built = FILTER.buildWhereSql(options.filterGroup, dialect);
      if (built.errors.length) return { status: 'rejected', message: built.errors.join(' ') };
      if (built.sql) { whereParts.push(built.sql); filtersApplied.push('Filter: ' + built.plainEnglish); }
    }
    ...
    if (options.existsFilter && options.existsFilter.relatedTable) {
      var existsErr = addExistsFilter(options.existsFilter);
      if (existsErr) return { status: 'rejected', message: existsErr };
    }
    if (Array.isArray(options.existsFilters)) {
      for (var ei = 0; ei < options.existsFilters.length; ei++) {
        var existsErr2 = addExistsFilter(options.existsFilters[ei]);
        if (existsErr2) return { status: 'rejected', message: existsErr2 };
      }
    }
