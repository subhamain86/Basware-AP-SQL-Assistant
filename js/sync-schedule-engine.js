(function (root) {
  ...
  function computeNextRun(optionId, fromDate) { var opt = getOption(optionId); if (!opt || opt.minutes == null) return null; var base = fromDate instanceof Date ? fromDate : new Date(); return new Date(base.getTime() + opt.minutes * 60000).toISOString(); }
  ...
  var API = { OPTIONS: OPTIONS, DEFAULT_OPTION_ID: DEFAULT_OPTION_ID, getOption: getOption, getDefaultOption: getDefaultOption, loadSelectedOptionId: loadSelectedOptionId, saveSelectedOptionId: saveSelectedOptionId, computeNextRun: computeNextRun, toIntervalMs: toIntervalMs };
