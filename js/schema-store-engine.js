(function (root) {
  ...
  function createStore(storageImpl) {
    ...
    function load() { if (!storageImpl) return; var raw; try { raw = storageImpl.getItem(STORAGE_KEY); } catch (e) { raw = null; } if (!raw) return; try { var parsed = JSON.parse(raw); if (parsed && Array.isArray(parsed.entries)) { state.entries = parsed.entries; state.activeId = parsed.activeId || null; } } catch (e) { state.entries = []; state.activeId = null; } }
    load();
    ...
    function importLegacySingleSchema(legacySchema, name) { var entry = createEntry({ name: name || (legacySchema && legacySchema.schema_name) || 'Migrated Schema', schema: legacySchema, source: 'embedded' }); state.entries = [entry]; state.activeId = entry.id; persist(); return entry; }
