(function (root) {
  ...
  function createRelationshipStore() {
    ...
    function setManualRelationship(fromTable, fromColumn, toTable, toColumn) { manual[key(fromTable, toTable)] = { fromTable: String(fromTable).toUpperCase(), fromColumn: String(fromColumn).toUpperCase(), toTable: String(toTable).toUpperCase(), toColumn: String(toColumn).toUpperCase() }; }
  ...
  function createEffectiveEngine(baseEngine, relationshipStore) {
    ...
    wrapped.findRelationship = function (tableA, tableB) { var real = baseEngine.findRelationship(tableA, tableB); if (real) return real; return relationshipStore ? relationshipStore.getManualRelationship(tableA, tableB) : null; };
    return wrapped;
  }
