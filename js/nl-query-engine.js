(function (root) {
  ...
  function cleanValue(raw) { if (raw == null) return ''; var v = String(raw).trim(); if ((v.charAt(0) === '"' && v.charAt(v.length - 1) === '"') || (v.charAt(0) === "'" && v.charAt(v.length - 1) === "'")) v = v.slice(1, -1); v = v.replace(/[.,;]+$/, ''); return v; }
  ...
  function matchFilters(text, engine, tableNames, now) {
    ...
    tableNames.forEach(function (tname) { var table = engine.getTable(tname); if (!table) return; table.columns.forEach(function (col) { var key = tname + '.' + col.name; if (usedColumns[key] || !Array.isArray(col.decode)) return; for (var i = 0; i < col.decode.length; i++) { var pair = col.decode[i]; var labelPhrase = normalizeSpaces(pair.label); if (labelPhrase.length > 2 && textLower.indexOf(labelPhrase) !== -1) { conditions.push({ table: tname, column: col.name, operator: 'eq', value: pair.code }); usedColumns[key] = true; break; } } }); });
    var lastDaysMatch = textLower.match(/last\s+(\d+)\s+days?/);
    if (lastDaysMatch && DATATYPE) { var n = parseInt(lastDaysMatch[1], 10); var dateCol = findBestDateColumn(engine, tableNames, usedColumns); if (dateCol) { var nowDate = now || new Date(); var cutoff = new Date(nowDate.getTime() - n * 24 * 60 * 60 * 1000); var iso = cutoff.toISOString().slice(0, 10); conditions.push({ table: dateCol.table, column: dateCol.column, operator: 'gte', value: iso }); usedColumns[dateCol.table + '.' + dateCol.column] = true; } }
    return conditions;
  }
  ...
  function matchHierarchy(text, engine, candidateTableNames) {
    ...
    if (!selfRefTables.length) return null; if (selfRefTables.length === 1) return selfRefTables[0];
  ...
  function describeOperatorForDisplay(c) {
    switch (c.operator) { case 'eq': return '= ' + c.value; case 'neq': return '<> ' + c.value; case 'gt': return '> ' + c.value; case 'lt': return '< ' + c.value; case 'gte': return '>= ' + c.value; case 'lte': return '<= ' + c.value; case 'contains': return 'contains "' + c.value + '"'; case 'not_contains': return 'does not contain "' + c.value + '"'; case 'starts_with': return 'starts with "' + c.value + '"'; case 'ends_with': return 'ends with "' + c.value + '"'; case 'between': return 'between ' + c.value + ' and ' + c.value2; case 'in': return 'is one of (' + c.value + ')'; case 'not_in': return 'is not one of (' + c.value + ')'; default: return String(c.operator); }
  }
  ...
  function matchColumnValueAssignments(searchText, engine, tableName) {
    var table = engine.getTable(tableName); if (!table) return []; var out = []; var used = {};
    table.columns.forEach(function (col) {
      if (used[col.name]) return; var phrases = [normalizeSpaces(col.name)]; if (col.alias) phrases.push(normalizeSpaces(col.alias));
      for (var p = 0; p < phrases.length; p++) {
  ...
  function interpretCrDescription(text, engine, opts) {
    ...
    var command = detectCrCommand(text); var ranked = scoreAllTables(text, engine).filter(function (s) { return s.score >= 1; });
    ...
    var matched = []; if (command) matched.push('Query Type: ' + command); if (table) matched.push('Table: ' + table);
  ...
  function singularize(word) { if (word.length > 4 && word.slice(-3) === 'ies') return word.slice(0, -3) + 'y'; if (word.length > 3 && word.slice(-2) === 'es' && /[sxz]es$|[cs]hes$/.test(word)) return word.slice(0, -2); if (word.length > 3 && word.slice(-1) === 's' && word.slice(-2) !== 'ss') return word.slice(0, -1); return word; }
  ...
  function scoreColumnAgainstPhrase(col, phraseNormalized) {
    if (!phraseNormalized) return 0; var score = 0;
    var namePhrase = normalizeSpaces(col.name); var aliasPhrase = col.alias ? normalizeSpaces(col.alias) : ''; var descPhrase = col.description ? normalizeSpaces(col.description) : '';
  ...
  function scoreAllTablesEnhanced(text, engine) {
    ...
    return base.map(function (entry) {
      ...
      var bareWords = normalizeWordSet(tokenize(bareTableName(t.name))); var nameOverlap = bareWords.filter(function (w) { return textContentWords.indexOf(w) !== -1; }).length;
  ...
  function matchColumnsEnhanced(text, engine, tableNames) {
    ...
    tableNames.forEach(function (tname) { var table = engine.getTable(tname); if (!table) return; table.columns.forEach(function (col) { var key = tname + '.' + col.name; if (existingKeys[key]) return; if (!col.description) return; var descWords = normalizeWordSet(tokenize(col.description).filter(function (w) { return w.length > 2 && !STOPWORDS[w]; })); if (!descWords.length) return; var overlap = textWords.filter(function (w) { return descWords.indexOf(w) !== -1; }).length; if (overlap >= 3) { var entry = { table: tname, column: col.name }; if (col.alias) entry.alias = col.alias; base.push(entry); existingKeys[key] = true; } }); });
    return base;
  }
  ...
  function resolveBooleanCode(engine, tableName, columnName, wantTrue) { var decode = engine.getValueMap(tableName, columnName); if (decode && decode.length) { var wordSet = wantTrue ? TRUE_LABEL_WORDS : FALSE_LABEL_WORDS; for (var i = 0; i < decode.length; i++) { if (wordSet.indexOf(String(decode[i].label).toLowerCase()) !== -1) return decode[i].code; } } return wantTrue ? '1' : '0'; }
  function matchBooleanFlagFilters(text, engine, tableNames) {
    ...
    tableNames.forEach(function (tname) {
      ...
      table.columns.forEach(function (col) {
        ...
        var tiedCandidates = []; tableNames.forEach(function (tn2) { var t2 = engine.getTable(tn2); if (!t2) return; t2.columns.forEach(function (c2) { var concept2 = deriveBooleanConcept(c2.name); if (concept2 && concept2.qualifier === qualifier) tiedCandidates.push({ table: tn2, column: c2.name, concept: concept2.concept }); }); });
  ...
  function matchMembershipFilters(text, engine, tableNames) {
    ...
    while ((m = re.exec(text))) {
      var value = normalizeSpaces(m[1]); if (!value) continue; var best = null, bestScore = 0;
  ...
  var AGG_PATTERNS = [ { fn: 'COUNT', re: /\b(?:count of|how many|number of)\s*([a-z0-9 _]*)/i }, { fn: 'SUM', re: /\b(?:sum of|total)\s+([a-z0-9 _]+)/i }, { fn: 'AVG', re: /\b(?:average|avg)\s+(?:of\s+)?([a-z0-9 _]+)/i }, { fn: 'MIN', re: /\b(?:minimum|lowest|smallest)\s+([a-z0-9 _]+)/i }, { fn: 'MAX', re: /\b(?:maximum|highest|largest)\s+([a-z0-9 _]+)/i } ];
  function matchAggregations(text, engine, tableNames) {
    var results = [];
    AGG_PATTERNS.forEach(function (p) { var m = text.match(p.re); if (!m) return; var phrase = normalizeSpaces(m[1] || ''); if (p.fn === 'COUNT' && (!phrase || !findColumnByPhraseScored(engine, tableNames, phrase, 5))) { results.push({ aggregate: 'COUNT', table: tableNames[0], column: '*', alias: 'record_count' }); return; } var best = findColumnByPhraseScored(engine, tableNames, phrase, 4); if (best) results.push({ aggregate: p.fn, table: best.table, column: best.column, alias: (p.fn.toLowerCase() + '_' + best.column.toLowerCase()) }); });
    return results;
  }
  ...
  function matchHaving(text, aggregates) {
    var m = String(text || '').match(/\bhaving\s+(count|sum|average|avg|total|minimum|maximum)\b(?:\s+of\s+[a-z0-9 _]+)?\s*(more than|greater than|at least|less than|at most|equal to|=)\s+([\d.]+)/i); if (!m) return null;
  ...
  function findAmbiguousTerms(text, engine, tableNames, resolvedFilters) {
    ...
    AMBIGUOUS_TRIGGER_WORDS.forEach(function (word) {
      if (textLower.indexOf(word) === -1) return; var candidates = [];
  ...
  function buildMatchedSummary(tables, columns, filters, orderBy, groupBy, aggregates, limit, distinct, bridgeTables) {
    ...
    if (limit) matched.push('Limit: ' + limit); if (distinct) matched.push('Remove duplicates: yes');
    return matched;
  }
  ...
  function interpretRequirement(text, engine, opts) {
    opts = opts || {}; var now = opts.now || new Date();
    var normalizedText = normalizeThousandsSeparators(String(text || '')); if (!normalizedText.trim()) return emptyRichInterpretation();
    ...
    var usedCols = {}; var boolResult = matchBooleanFlagFilters(normalizedText, engine, candidateNames); boolResult.filters.forEach(function (f) { usedCols[f.table + '.' + f.column] = true; });
    ...
    var orderBy = matchSortEnhanced(normalizedText, engine, candidateNames); var limit = matchLimit(normalizedText); var distinct = matchDistinct(normalizedText);
    var aggregates = matchAggregations(normalizedText, engine, candidateNames); var groupBy = matchGroupBy(normalizedText, engine, candidateNames); var having = matchHaving(normalizedText, aggregates);
    ...
    var warnings = []; if (closure.unresolved.length) warnings.push('Could not automatically connect: ' + closure.unresolved.join(', ') + '. You can connect these manually in Advanced Options.');
