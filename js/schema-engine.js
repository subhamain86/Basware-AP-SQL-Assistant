(function (root) {
  ...
  function createEngine(schema) {
    ...
    function getValueMap(tableName, columnName) { var c = getColumn(tableName, columnName); return (c && Array.isArray(c.decode) && c.decode.length) ? c.decode : null; }
    function findRelationship(tableA, tableB) {
      var ta = getTable(tableA), tb = getTable(tableB);
      if (!ta || !tb) return null;
      for (var i = 0; i < ta.columns.length; i++) { var c = ta.columns[i]; if (c.foreign_key && String(c.foreign_key.table).toUpperCase() === String(tb.name).toUpperCase()) return { fromTable: ta.name, fromColumn: c.name, toTable: tb.name, toColumn: c.foreign_key.column }; }
      for (var j = 0; j < tb.columns.length; j++) { var c2 = tb.columns[j]; if (c2.foreign_key && String(c2.foreign_key.table).toUpperCase() === String(ta.name).toUpperCase()) return { fromTable: tb.name, fromColumn: c2.name, toTable: ta.name, toColumn: c2.foreign_key.column }; }
      return null;
    }
    function getSelfReferencingEdges(tableName) {
      var t = getTable(tableName); if (!t) return [];
      var edges = [];
      t.columns.forEach(function (c) { if (c.foreign_key && String(c.foreign_key.table).toUpperCase() === String(t.name).toUpperCase()) edges.push({ fromColumn: c.name, toColumn: c.foreign_key.column }); });
      return edges;
    }
    function getStatus() {
