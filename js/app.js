(function () {
  ...
  var TOURS = {
    /* ============================================================================
   AP-SQL ASSISTANT — V10.8 GUIDED WALKTHROUGH & RESPONSIVE UI PATCH
   ----------------------------------------------------------------------------
   HOW TO APPLY (see full instructions in chat)
   1. Open ap-sql-assistant/js/app.js
   2. Find "var TOURS = {" near the end of the file and select everything
      from there down to (but NOT including) the file's final "})();"
   3. Delete that selection and paste this entire file's contents in its place.
   4. Save. Then also apply styles-v10.8-additions.css to styles.css.
   ========================================================================= */
  ...
  var TOURS = {
    quickstart: [
      ...
      tourStep('[data-view="crbuilder"]', 'bottom', 'Query Builder for CR',
        ...
        'This only ever produces SQL text for review — it never runs anything against a real database.'),
  ...
  function clampToViewport(top, left, popW, popH, minTop) {
    ...
    return {
      top: Math.min(Math.max(lowestTop, top), Math.max(lowestTop, vh - popH - TOUR_MARGIN)),
      left: Math.min(Math.max(TOUR_MARGIN, left), Math.max(TOUR_MARGIN, vw - popW - TOUR_MARGIN))
    };
  }
  function choosePlacement(rect, popW, popH, preferred, navbarOffset) {
    var vw = window.innerWidth, vh = window.innerHeight;
    var space = {
      top: Math.max(0, rect.top - navbarOffset - TOUR_GAP),
      bottom: Math.max(0, vh - rect.bottom - TOUR_GAP),
      left: Math.max(0, rect.left - TOUR_GAP),
      right: Math.max(0, vw - rect.right - TOUR_GAP)
    };
  ...
  function computeVisibleProgress() {
    ...
    for (var i = 0; i < TOUR.length; i++) {
      var t = document.querySelector(TOUR[i].sel);
      if (t && isElementVisible(t)) visibleIndices.push(i);
    }
  ...
  function ensureProgressBar() {
    ...
    bar.innerHTML = '<div class="tour-progressbar-fill"></div>';
    var stepLabel = $('tourStepLabel');
    if (stepLabel && stepLabel.parentNode) stepLabel.parentNode.insertBefore(bar, stepLabel);
  }
  function scrollTargetIntoView(target, navbarOffset) {
    ...
    var desired = Math.max(0, elTop - navbarOffset - 32);
  ...
  function positionTourAttempt(skipCount, direction) {
    ...
    ensureProgressBar();
    renderTourChrome(step);
    var navbarOffset = getNavbarOffset();
    var scrolled = scrollTargetIntoView(target, navbarOffset);
    setTimeout(function () {
      ...
      if (isMobileTourViewport()) {
        popup.classList.add('tour-mobile-sheet');
  ...
  function scheduleTourReposition() {
    if (!tourOpen || tourResizeScheduled) return;
    tourResizeScheduled = true;
    (window.requestAnimationFrame || function (fn) { setTimeout(fn, 16); })(function () { tourResizeScheduled = false; if (tourOpen) positionTour(); });
  }
